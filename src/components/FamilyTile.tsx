...
export function FamilyTile({ assignment, companionshipId }: FamilyTileProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`group flex items-center gap-2 px-2 py-1.5 bg-amber-50 border border-amber-200 rounded-lg cursor-grab active:cursor-grabbing select-none ${
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {person?.address && (
          <div className="text-xs text-gray-500 truncate">{person.address}</div>
        )}
      </div>
      {canRemove && (
        <button
          ...
          className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded-full hover:bg-amber-200 text-amber-400 hover:text-amber-600"
